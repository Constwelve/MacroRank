...
from src.util import InversePairs, kendall, normalization, rank, top_k_match
...
checkp = torch.load(args.checkp, map_location='cpu')
...
for design in dataset.raw_file_names:
    ...
    labels.extend(np.loadtxt(label_p)[np.loadtxt(idx_p,dtype=int)].tolist())

meann = np.mean(labels, 0 )
maxx = np.max(labels, 0 )
minn = np.min(labels, 0 )

print("\t\t\ttop10\ttop20\ttop30")
dataset.mode = 'CNN'
mres = 0
with torch.no_grad():
    designs = []
    ...
    for design in args.tests:
        ...
        for i, data in enumerate(test_loader):
            ...
            #data.y[:, 1 : ] =  data.y[:, 1 : ].cpu() * (maxx - minn) + meann
        ...
        print(mae)
        mres += mae
        #np.savetxt('preds_{}.txt'.format(args.model), preds, fmt='%f')
        #plt.scatter(reals, preds)
        #plt.savefig('visual_{}/{}.png'.format(args.model, design))
        #plt.cla()
        #plt.scatter(rank(reals), rank(preds))
        #plt.savefig('visual_{}/{}_rank.png'.format(args.model, design))
        #plt.cla()
    print(mres/len(args.tests))
    #logger.add_embedding(torch.cat(embdds, dim=0), metadata=np.array(designs))
